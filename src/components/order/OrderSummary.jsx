              ...
              <div className='flex gap-[15px] pb-[15px]' style={{ borderBottom: "1px solid #a3a3a3a3" }} name="cartItems">
                ...
                <div className='flex flex-1 justify-between'>
                  <div className='flex flex-col'>
                    <h3 className='text-[#4A4B4D] text-[18px] font-bold' name='dishName'>{item.dish.name}</h3>
                    {item.toppings.map((topping) => (
                      <p className='text-[#a4a5a8]' name="toppingName">{topping.name}</p>
                    ))}
                  </div>
                  <span className='text-[#4A4B4D]' name="price">{totalPrice.toFixed(0)}đ</span>
                </div>
              </div>
            );
          })}
